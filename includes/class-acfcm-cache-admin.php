...
final class ACFCM_Cache_Admin {
    ...
    public static function execute( $action, $approval_id = '' ) {
        ...
        if ( 'security' === $action ) {
            $r = ACFCM_Defense::install( $zone, array( 'complete_security' => true ) );
    ...
    public static function network_site_buttons( array $site ) {
        ...
        $actions = array( $pending ? 'step' : 'install' => $pending ? 'Resume cache installation' : 'Install cache', 'security' => 'Install security rules' );
    ...
    public static function policy() {
        ...
        if ( ! $j || in_array( $j['status'], array( 'complete', 'rolled_back' ), true ) ) {
            ...
            self::form( 'install', 'Install cache', null, true );
        }
