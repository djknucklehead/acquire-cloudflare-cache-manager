...
final class ACFCM_Cache_Admin {
    ...
    public static function execute( $action, $approval_id = '' ) {
        $zone = Acquire_Cloudflare_Cache_Manager::get_zone_id();
        if ( 'security' === $action && ACFCM_Defense::excluded_identity( $zone ) ) {
            try { $review = ACFCM_Defense::review_excluded( $zone ); }
            catch ( RuntimeException $e ) { update_option( 'acfcm_security_result', array( 'success' => false, 'message' => $e->getMessage() ), false ); throw $e; }
            update_option( 'acfcm_security_result', array( 'success' => false, 'message' => 'Current zone is active and unpaused. Review the security additions below and approve onboarding. Cache rules are unchanged.' ), false );
            return array( 'status' => 'review' );
        }
        if ( in_array( $action, array( 'security', 'security_approve' ), true ) ) {
            $r = ACFCM_Defense::install( $zone, array( 'defense_baseline' => true ), null, 'security_approve' === $action ? $approval_id : null );
            ...
            if ( 'security_approve' === $action ) { $review = get_option( 'acfcm_security_review', array() ); $review['completed'] = true; update_option( 'acfcm_security_review', $review, false ); }
    ...
    public static function ajax() {
        self::authorize();
        try { wp_send_json_success( self::execute( sanitize_key( $_POST['policy_action'] ?? '' ), sanitize_text_field( wp_unslash( $_POST['review_id'] ?? '' ) ) ) ); }
    ...
    public static function handle() {
        ...
        try { $r = self::execute( sanitize_key( $_POST['policy_action'] ?? '' ), sanitize_text_field( wp_unslash( $_POST['review_id'] ?? '' ) ) ); update_option( 'acfcm_cache_notice', $r['error'] ?? 'Cache policy ' . $r['status'] . '.', false ); }
    ...
    public static function network_execute( array $request ) {
        ...
        if ( ! in_array( $action, array( 'install', 'security', 'security_approve', 'step', 'export' ), true ) ) { throw new RuntimeException( 'Unknown zone action.' ); }
        switch_to_blog( $id );
        try {
            if ( 'export' === $action ) {
            ...
            return self::execute( $action, sanitize_text_field( $request['review_id'] ?? '' ) );
    ...
    public static function network_site_buttons( array $site ) {
        ...
        $review = get_blog_option( $id, 'acfcm_security_review', array() );
        if ( $review && empty( $review['completed'] ) && $review['zone'] === $zone && (int) $review['user'] === get_current_user_id() ) {
            self::security_review_details( $review );
            $actions['security_approve'] = 'Approve security onboarding';
        }
    ...
    public static function network_zones( array $sites ) {
        ...
        foreach ( $sites as $site ) {
            ...
            $id = (int) $site['blog_id']; $zone = $site['zone_id'];
            foreach ( array( 'install', 'step', 'security', 'security_approve', 'export' ) as $action ) {
                $form = 'acfcm-zone-' . $id . '-' . $action;
                ...
                if ( 'security_approve' === $action ) { $review = get_blog_option( $id, 'acfcm_security_review', array() ); echo '<input type="hidden" name="review_id" value="' . esc_attr( $review['id'] ?? '' ) . '">'; }
    ...
    private static function security_review_details( array $review ) {
        echo '<p><strong>Security review: ' . esc_html( $review['identity']['name'] ) . '</strong> — active and unpaused when checked. Account: ' . esc_html( $review['identity']['account'] ) . '. Approval rechecks status and rules. Existing rules and cache configuration are retained.</p><details><summary>Security additions to approve</summary><pre style="white-space:pre-wrap">' . esc_html( wp_json_encode( $review['plan'], JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES ) ) . '</pre></details>';
    }
    private static function form( $action, $label, $preview = null, $migration = false ) {
        ...
        if ( 'security_approve' === $action ) { $review = get_option( 'acfcm_security_review', array() ); echo '<input type="hidden" name="review_id" value="' . esc_attr( $review['id'] ?? '' ) . '">'; }
        if ( $preview ) {
    ...
    public static function policy() {
        ...
        $review = get_option( 'acfcm_security_review', array() );
        if ( $review && empty( $review['completed'] ) && $review['zone'] === $zone && (int) $review['user'] === get_current_user_id() ) { self::security_review_details( $review ); self::form( 'security_approve', 'Approve security onboarding', null, true ); }
