...
final class ACFCM_Network_Queue {
    ...
    public static function render() {
        ...
        echo '<p>' . esc_html( ( $s['cancelled'] ? 'Cancelled' : ( $s['paused'] ? 'Paused' : 'Active / idle' ) ) . ' | Waiting for origin: ' . ( $counts['origin'] ?? 0 ) . ' | Waiting for Cloudflare: ' . ( $counts['edge'] ?? 0 ) . ' | Accepted: ' . ( $counts['done'] ?? 0 ) . ' | Failed: ' . ( $counts['failed'] ?? 0 ) . ' | Cancelled: ' . ( $counts['cancelled'] ?? 0 ) . ' | Updates/quiet period until: ' . gmdate( 'Y-m-d H:i:s', self::session_due( $s ) ) . ' UTC | Next eligible: ' . ( self::next_due( $s ) ? gmdate( 'Y-m-d H:i:s', self::next_due( $s ) ) . ' UTC' : 'none' ) ) . '</p>';
