...
for mode,port in [('single',18891),('multi',18892)]:
 ...
 plugin=root/'wp-content/plugins/acquire-cloudflare-cache-manager';plugin.mkdir();(plugin/'acquire-cloudflare-cache-manager.php').symlink_to(repo/'acquire-cloudflare-cache-manager.php');(plugin/'includes').symlink_to(repo/'includes',target_is_directory=True);(plugin/'assets').symlink_to(repo/'assets',target_is_directory=True)
